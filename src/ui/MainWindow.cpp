...
void MainWindow::setupSidebar() {
  ...
  sidebarMenu->setStyleSheet("QTreeWidget {"
  ...
  connect(sidebarMenu, &QTreeWidget::itemClicked, this,
          &MainWindow::onSidebarItemClicked);
}
...
QWidget *MainWindow::createWarehouseSection() {
  ...
  sectionTitle->setStyleSheet("QLabel {"
                              "    font-size: 16pt;"
                              "    font-weight: bold;"
                              "    color: #333;"
                              "    padding: 10px 0;"
  ...
  categoryComboBox->setStyleSheet("QComboBox {"
                                  "    border: 2px solid #ddd;"
                                  "    border-radius: 5px;"
                                  "    padding: 5px 10px;"
                                  "    font-size: 14px;"
...
void MainWindow::setupTable() {
  ...
  connect(actionsDelegate, &ActionsDelegate::editRequested, this,
          &MainWindow::editProductByRow);
  connect(actionsDelegate, &ActionsDelegate::deleteRequested, this,
          &MainWindow::deleteProductByRow);
  connect(actionsDelegate, &ActionsDelegate::writeOffRequested, this,
          &MainWindow::writeOffProductByRow);
}
...
void MainWindow::addProduct() {
  ...
  if (dialog.exec() == QDialog::Accepted) {
    try {
      ...
      QString idError =
          ProductValidator::validateId(*inventoryManager, product.getId());
      if (!idError.isEmpty()) {
      ...
      dbManager->products().addProduct(product);
      ...
      QMessageBox::warning(this, "Error",
                           QString("Failed to add product: %1").arg(e.what()));
    } catch (...) {
...
void MainWindow::createOrder() {
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    OrderService::Result result =
        OrderService::createOrder(*inventoryManager, *dbManager, order);
...
void MainWindow::saveOrderHistoryToTxt(const Order &order) {
  ...
  if (file.open(QIODevice::WriteOnly | QIODevice::Append | QIODevice::Text)) {
    ...
    out << "Total Amount: $" << QString::number(order.getTotalAmount(), 'f', 2)
        << "\n";
    out << "Total Discount: $"
        << QString::number(order.getTotalDiscount(), 'f', 2) << "\n";
    out << "---\n\n";
...
void MainWindow::editOrder(int orderId) {
  Order order = dbManager->orders().getOrder(orderId);
  if (order.getId() == 0) {
  ...
  if (dialog.exec() == QDialog::Accepted) {
    ...
    if (dbManager->orders().updateOrder(updatedOrder)) {
      QMessageBox::information(
          this, "Success",
          QString("Order #%1 updated successfully!").arg(updatedOrder.getId()));
...
void MainWindow::exportReport() {
  QString filename = QFileDialog::getSaveFileName(this, "Export Report", "",
                                                  "Text Files (*.txt)");
  if (!filename.isEmpty()) {
    if (FileManager::exportReportToText(*inventoryManager,
...
void MainWindow::applyFilters() {
  ...
  auto products = ProductFilterService::filterProducts(*inventoryManager,
                                                       category, searchText);
...
QWidget *MainWindow::createOrdersSection() {
  ...
  QPushButton *viewHistoryBtn =
      new QPushButton("View Order History", sectionWidget);
  ...
  ordersTable->setHorizontalHeaderLabels(
      QStringList() << "ID" << "Company" << "Contact" << "Type" << "Date"
                    << "Total" << "Actions");
  ...
  auto populateOrdersTable = [this, ordersTable](QTableWidget *table) {
    table->setRowCount(0);
    auto orders = dbManager->orders().getAllOrders();
    table->setRowCount(orders.size());
    for (size_t i = 0; i < orders.size(); ++i) {
      const auto &order = orders[i];
      table->setItem(i, 0,
                     new QTableWidgetItem(QString::number(order.getId())));
      ...
      table->setItem(
          i, 4,
          new QTableWidgetItem(order.getOrderDate().toString("yyyy-MM-dd")));
      table->setItem(i, 5,
                     new QTableWidgetItem(QString("$%1").arg(
                         order.getTotalAmount(), 0, 'f', 2)));
      ...
      connect(editBtn, &QPushButton::clicked, this,
              [this, orderId]() { editOrder(orderId); });
      ...
      connect(deleteBtn, &QPushButton::clicked, this, [this, orderId]() {
        ...
        if (ret == QMessageBox::Yes) {
          if (dbManager->orders().deleteOrder(orderId)) {
            QMessageBox::information(this, "Success",
  ...
  ordersTable->horizontalHeader()->setSectionResizeMode(
...
QWidget *MainWindow::createInventorySection() {
  ...
  for (size_t i = 0; i < products.size(); ++i) {
    ...
    if (product) {
      inventoryModel->setItem(
          i, 0, new QStandardItem(QString::number(product->getId())));
      inventoryModel->setItem(
          i, 1, new QStandardItem(QString::fromStdString(product->getName())));
      inventoryModel->setItem(
          i, 2,
          new QStandardItem(QString::fromStdString(product->getCategory())));
      QStandardItem *currentQtyItem =
          new QStandardItem(QString::number(product->getQuantity()));
      currentQtyItem->setEditable(false);
      inventoryModel->setItem(i, 3, currentQtyItem);
      QStandardItem *actualItem =
          new QStandardItem(QString::number(product->getQuantity()));
  ...
  inventoryTableView->setItemDelegateForColumn(
      4, new QStyledItemDelegate(sectionWidget));
  ...
  connect(inventoryModel, &QStandardItemModel::dataChanged, this,
          [inventoryModel](const QModelIndex &topLeft,
            ...
            if (topLeft.column() == 4) {
              for (int row = topLeft.row(); row <= bottomRight.row(); ++row) {
                ...
                if (currentQtyItem && actualQtyItem && diffItem) {
                  ...
                  if (difference > 0) {
                    diffItem->setForeground(QBrush(QColor("#4CAF50")));
                  } else if (difference < 0) {
                    diffItem->setForeground(QBrush(QColor("#f44336")));
                  } else {
                    diffItem->setForeground(QBrush(QColor("#333")));
                  }
                }
              }
            }
  ...
  connect(cancelBtn, &QPushButton::clicked, this, [this, inventoryModel]() {
    if (inventoryManager) {
      ...
      for (size_t i = 0; i < products.size() &&
                         i < static_cast<size_t>(inventoryModel->rowCount());
           ++i) {
        const auto &product = products[i];
        if (product) {
          QStandardItem *actualItem =
              inventoryModel->item(static_cast<int>(i), 4);
          if (actualItem) {
  ...
  connect(
      ...
      [this, inventoryModel, inventoryTableView]() {
        if (!inventoryManager) {
          QMessageBox::warning(this, "Error",
                               "Inventory manager is not available.");
          return;
        }
        ...
        for (int i = 0; i < inventoryModel->rowCount(); ++i) {
          ...
          try {
            InventoryAdjustmentService::applyAdjustment(
                *inventoryManager, dbManager, id, currentQty, actualQty,
                result);
          } catch (const ProductException &e) {
            QMessageBox::warning(this, "Error",
        ...
        if (result.itemsUpdated > 0) {
          ...
          FileManager::saveToBinary(*inventoryManager,
                                    dataFilePath.toStdString());
          ...
          for (size_t i = 0;
            ...
            if (product) {
              QStandardItem *currentQtyItem =
                  inventoryModel->item(static_cast<int>(i), 3);
              QStandardItem *actualQtyItem =
                  inventoryModel->item(static_cast<int>(i), 4);
              if (currentQtyItem) {
                currentQtyItem->setText(
...
QWidget *MainWindow::createReportsSection() {
  ...
  reportsSidebar->setStyleSheet("QWidget {"
  ...
  QPushButton *writeOffsBtn =
      new QPushButton("Write-offs Report", reportsSidebar);
  QPushButton *inventoryReportBtn =
      new QPushButton("Current Inventory", reportsSidebar);
  QPushButton *financialReportBtn =
      new QPushButton("Financial Report", reportsSidebar);
  ...
  writeOffsTitle->setStyleSheet("QLabel {"
                                "    font-size: 14pt;"
                                "    font-weight: bold;"
                                "    color: #333;"
                                "    padding: 10px 0;"
                                "}");
  QPushButton *exportWriteOffsBtn =
      new QPushButton("Export to TXT", writeOffsPage);
  ...
  connect(exportWriteOffsBtn, &QPushButton::clicked, this, [this]() {
    QString writeOffsText = updateWriteOffsReport();
    QString fileName =
        QFileDialog::getSaveFileName(this, "Export Write-offs Report", "",
  ...
  QLabel *inventoryTitle =
      new QLabel("Current Inventory Report", inventoryReportPage);
  inventoryTitle->setStyleSheet("QLabel {"
                                "    font-size: 14pt;"
                                "    font-weight: bold;"
                                "    color: #333;"
                                "    padding: 10px 0;"
  ...
  QPushButton *exportInventoryBtn =
      new QPushButton("Export to TXT", inventoryReportPage);
  ...
  connect(
      exportInventoryBtn, &QPushButton::clicked, this, [inventoryText, this]() {
        QString fileName =
            QFileDialog::getSaveFileName(this, "Export Inventory Report", "",
                                         "Text Files (*.txt);;All Files (*)");
        if (!fileName.isEmpty()) {
          QFile file(fileName);
          if (file.open(QIODevice::WriteOnly | QIODevice::Text)) {
  ...
  salesTitleLabel->setStyleSheet("QLabel {"
                                 "    font-size: 16pt;"
                                 "    font-weight: normal;"
                                 "    color: #666;"
                                 "}");
  ...
  QLabel *salesLabel =
      new QLabel("$" + QString::number(calculateTotalSales(), 'f', 2));
  salesLabel->setStyleSheet("QLabel {"
                            "    font-size: 24pt;"
                            "    font-weight: bold;"
                            "    color: #2196F3;"
                            "}");
  ...
  categoryChartWidget->setStyleSheet("QWidget {"
                                     "    background-color: white;"
                                     "    border-radius: 8px;"
  ...
  QLabel *categoryChartTitle =
      new QLabel("Category Distribution", categoryChartWidget);
  categoryChartTitle->setStyleSheet("QLabel {"
                                    "    font-size: 16pt;"
                                    "    font-weight: bold;"
                                    "    color: #333;"
                                    "    padding: 5px 0;"
  ...
  if (!categoryData.isEmpty()) {
    ...
    QLabel *noCategoryDataLabel =
        new QLabel("No category data available.\nCreate orders with products "
                   "to see the chart.",
                   categoryChartWidget);
    noCategoryDataLabel->setAlignment(Qt::AlignCenter);
    noCategoryDataLabel->setStyleSheet("QLabel {"
                                       "    font-size: 14px;"
                                       "    color: #999;"
                                       "    padding: 40px;"
                                       "}");
  ...
  companiesChartWidget->setStyleSheet("QWidget {"
                                      "    background-color: white;"
                                      "    border-radius: 8px;"
                                      "    border: 1px solid #e0e0e0;"
                                      "}");
  ...
  QLabel *topCompaniesTitle =
      new QLabel("Top 5 Companies by Purchases", companiesChartWidget);
  topCompaniesTitle->setStyleSheet("QLabel {"
                                   "    font-size: 16pt;"
                                   "    font-weight: bold;"
                                   "    color: #333;"
                                   "    padding: 5px 0;"
  ...
  if (!topCompanies.isEmpty()) {
    ...
    QLabel *noCompaniesDataLabel = new QLabel(
        "No company data available.\nCreate orders to see the chart.",
        companiesChartWidget);
    noCompaniesDataLabel->setAlignment(Qt::AlignCenter);
    noCompaniesDataLabel->setStyleSheet("QLabel {"
                                        "    font-size: 14px;"
                                        "    color: #999;"
                                        "    padding: 40px;"
                                        "}");
  ...
  connect(inventoryReportBtn, &QPushButton::clicked,
          [reportsStack]() { reportsStack->setCurrentIndex(1); });
  connect(financialReportBtn, &QPushButton::clicked,
          [reportsStack]() { reportsStack->setCurrentIndex(2); });
...
QList<QPair<QString, double>> MainWindow::getTopCompaniesData(int topCount) {
  ...
  std::sort(
      companyList.begin(), companyList.end(),
