...
QGroupBox *SalesReportDialog::createReportTableGroupBox() {
  ...
  reportTable->setHorizontalHeaderLabels(
      {"ID", "Company", "Contact", "Type", "Date", "Items", "Amount"});
...
void SalesReportDialog::generateReport() {
  ...
  switch (reportType) {
  case 0:
    currentOrders = dbManager->orders().getOrdersByDateRange(startDate, endDate);
    break;
  case 1:
    currentOrders = dbManager->orders().getOrdersByType(OrderType::RETAIL);
    break;
  case 2:
    currentOrders = dbManager->orders().getOrdersByType(OrderType::WHOLESALE);
    break;
  case 3:
    currentOrders = dbManager->orders().getAllOrders();
    break;
  case 4:
    currentOrders = dbManager->orders().getOrdersByDateRange(startDate, endDate);
    break;
...
void SalesReportDialog::refreshReport() {
  ...
  for (const auto &order : currentOrders) {
    ...
    reportTable->setItem(row, 0,
                         new QTableWidgetItem(QString::number(order.getId())));
    reportTable->setItem(row, 1, new QTableWidgetItem(order.getCompanyName()));
    reportTable->setItem(row, 2,
                         new QTableWidgetItem(order.getContactPerson()));
    reportTable->setItem(row, 3,
                         new QTableWidgetItem(order.getOrderTypeString()));
    reportTable->setItem(
        row, 4,
        new QTableWidgetItem(order.getOrderDate().toString("MM/dd/yyyy")));
    reportTable->setItem(
        row, 5, new QTableWidgetItem(QString::number(order.getItems().size())));
    reportTable->setItem(row, 6,
...
void SalesReportDialog::calculateStatistics() {
  ...
  totalRetailLabel->setText(QString("Retail: $%1 (%2 orders)")
                                .arg(totalRetail, 0, 'f', 2)
                                .arg(retailCount));
  totalWholesaleLabel->setText(QString("Wholesale: $%1 (%2 orders)")
                                   .arg(totalWholesale, 0, 'f', 2)
                                   .arg(wholesaleCount));
  totalAmountLabel->setText(QString("Total: $%1").arg(totalAmount, 0, 'f', 2));
  totalDiscountLabel->setText(
      QString("Discounts: $%1").arg(totalDiscount, 0, 'f', 2));
  ordersCountLabel->setText(QString("Orders: %1").arg(currentOrders.size()));
}

void SalesReportDialog::exportReport() {
  QString fileName = QFileDialog::getSaveFileName(
      this, "Export Report", "", "Text Files (*.txt);;All Files (*)");
  if (fileName.isEmpty()) {
    return;
  }
